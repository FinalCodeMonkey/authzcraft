...
public class AccessPolicyResponse {
    ...
    public static AccessPolicyResponse from(AccessPolicy policy) {
        AccessPolicyResponse response = new AccessPolicyResponse();
        response.setId(String.valueOf(policy.getId()));
        response.setTenantKey(policy.getTenantKey());
        response.setAppKey(policy.getAppKey());
        response.setPolicyKey(policy.getPolicyKey());
        response.setDisplayName(policy.getDisplayName());
        response.setDescription(policy.getDescription());
        response.setTargetRelationId(String.valueOf(policy.getTargetRelationId()));
        response.setOperationCode(policy.getOperationCode());
        response.setEffectKind(policy.getEffectKind());
        response.setLifecycleState(policy.getLifecycleState());
        response.setRuleBlueprintDisplayName(policy.getRuleBlueprintDisplayName());
        return response;
    }
